...
interface Props {
  prescription: IPrescriptionForPrint;
}
...
const PrescriptionPDF = ({ prescription }: Props) => {
  ...
  const date = new Date(prescription.prescription.person.birthDate);
  ...
  const hasAddress = Boolean(prescription.patientAddress?.cep);

  for (const prop of Object.keys(prescription?.patientAddress)) {
    if (!prescription?.patientAddress[prop]) {
      hasEmptyAddressInfo = true;
    }
  }
  const addressIsEmpty = hasEmptyAddressInfo && !hasAddress;
      ...
      >
        ...
        <View>
          ...
          <Text style={styles.cpf}>
            {`${
              prescription.prescription.person.hasResponsible
                ? `CPF responsável: ${formatToCPF(
                    prescription.prescription.person.responsibleCPF,
                  )}`
                : `${
                    prescription.prescription.person.cpf
                      ? `CPF: ${formatToCPF(
                          prescription.prescription.person.cpf,
                        )}`
                      : ``
              ...
              prescription.prescription.person.sex
            } Data de nascimento: ${birthYear} Idade: ${age} anos`}
          </Text>
          {addressIsEmpty ? (
            ''
          ) : (
            <Text style={styles.address}>
              Endereço: {prescription.patientAddress.street},{' '}
              {prescription.patientAddress.number},{' '}
              {prescription.patientAddress.city}{' '}
              {prescription.patientAddress.state},{' '}
              {prescription.patientAddress.cep}
            </Text>
          )}
        </View>
        ...
        <View style={styles.manualSignature}>
          ...
          <Text style={styles.date}>
            {prescription.prescription.shouldShowDate
              ? `${formatExtendedDate(
                  new Date(prescription?.prescription?.createDate),
                )},`
              : ''}{' '}
            {`${prescription.doctor.address.city} - ${prescription.doctor.address.state}`}
          </Text>
          <View
            style={{
              width: '100%',
...
export const generatePDF = async (prescription: IPrescriptionForPrint) => {
  try {
    const fileName = generateFileName(
      String(prescription.prescription.id),
      prescription.doctor.name,
      prescription.prescription.person.name,
    );

    console.log(prescription.prescription.documentTypeId);
